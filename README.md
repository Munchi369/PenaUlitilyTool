...
* 選手ビューを追加
* 選手検索で、通算成績、年度指定、ドラフト、タイトル、装備などの詳細条件に対応
* 検索条件プリセットの保存、読み込み、削除に対応
* ブックメーカーに単勝・複勝の専用マークカードを追加
* 装備設定時に `SOUGOU` を再計算するように変更

## スクリーンショット

### ver2.2.0 選手ビュー
<img width="500" height="450" alt="image" src="https://github.com/user-attachments/assets/5ebb9138-1b64-4602-8d5f-6d655292543d" />
...
* [打順自動編成](#14-打順自動編成)
* [監督ビルダー](#15-監督ビルダー)
* [SOUGOU再計算](#16-sougou再計算)
* [ブックメーカー](#17-ブックメーカー)
* [収支グラフ](#18-収支グラフ)
* [データベースメンテナンス](#19-データベースメンテナンス)
...
引退済みの選手を検索し、削除リストで確認してから削除できます。
出場機会の少ない退団選手を整理したいときに使います。
...
月間MVP、特別記録、装備中データなどがある選手は保護されます。
必要な場合だけ保護チェックを外してください。
実行前に必ず `penanto3` フォルダをバックアップしてください。
...
自チーム、お気に入り、名前検索の選手一覧を確認できます。
一覧から選手を選び、能力、プロフィール、年度別成績、通算成績を確認できます。
...
* 年度指定や通算成績を使った検索
* 検索条件のプリセット保存、読み込み、削除
* ダブルクリックで選手詳細を表示

検索結果は必要になったタイミングで読み込むため、初回表示が重くなりにくい作りになっています。
...
成績一覧と能力一覧は、条件を切り替えて並び替えやCSV出力ができます。
選手名鑑は、選択年度の一軍・二軍在籍選手をHTMLで出力します。
出力先は `doc/player-directory/` です。
...
装備を変更した場合は、装備効果を反映した `SOUGOU` も再計算されます。
...
自チームの通常打順とDH打順を、条件に合わせて自動で組み直します。
保存前にプレビューで変更内容を確認できます。

指定できる条件：

* 候補範囲: スタメンのみ / 一軍野手まで / 一軍＋二軍野手まで
* 守備適用: メイン守備のみ / サブ守備も考慮
* 評価軸: 総合 / 打撃 / 守備 / 機動力
* 打順理論: 王道型、2番最強、繋ぎの4番など
* 選手傾向: 若手重視、ベテラン重視、年俸重視など
...
通常は、上記の [装備管理](#11-装備管理) 画面から追加・編集してください。
